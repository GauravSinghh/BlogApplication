...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<MyErrorDetails> tutorialNotFoundException(CommentNotfoundException tnf ,WebRequest wr){
		MyErrorDetails err = new MyErrorDetails(LocalDateTime.now(), tnf.getMessage(), wr.getDescription(false));
		return new ResponseEntity<MyErrorDetails>(err, HttpStatus.BAD_REQUEST);
	}
	@ExceptionHandler(PostNotfoundException.class)
	public ResponseEntity<MyErrorDetails> PostNotfoundException(PostNotfoundException tnf ,WebRequest wr){
		MyErrorDetails err = new MyErrorDetails(LocalDateTime.now(), tnf.getMessage(), wr.getDescription(false));
		return new ResponseEntity<MyErrorDetails>(err, HttpStatus.BAD_REQUEST);
	}
